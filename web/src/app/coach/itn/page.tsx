...
export default function ITNTalentPage() {
  ...
  return (
    <>
      <Topbar
        title="ITN & Talent Test"
        breadcrumb={[
          { label: "Workspace" },
          { label: player.name },
          { label: tab === "itn" ? "ITN Test" : "Talent Test" },
        ]}
        actions={
          <>
            <div
            ...
            <GhostBtn icon="file-type-pdf">Export PDF</GhostBtn>
            <PrimaryBtn icon="device-floppy">Save</PrimaryBtn>
          </>
        }
      />
      <Styles />
      <div className="r-page">
        <div className="r-tabs" style={{ marginBottom: 14 }}>
          <button className={"r-tab " + (tab === "itn" ? "on" : "")} onClick={() => setTab("itn")}>
            <Icon name="checkup-list" /> ITN Test
          </button>
          <button
            className={"r-tab " + (tab === "talent" ? "on" : "")}
            onClick={() => setTab("talent")}
          >
            <Icon name="sparkles" /> Talent Test
          </button>
        </div>
