  ...
  return (
    <>
      <Topbar
        ...
        breadcrumb={[{ label: "Workspace" }, { label: "Finance" }]}
        actions={
          <>
            <GhostBtn icon="file-export">Export report</GhostBtn>
            <GhostBtn icon="receipt">Billing settings</GhostBtn>
            <PrimaryBtn icon="refresh">Renew now</PrimaryBtn>
          </>
        }
      />
      <Styles />
      <div className="r-page">
        <div className="fin-kpis">
          {FIN.kpis.map((k) => (
            <div key={k.k} className="fin-kpi">
              ...
              <div>
                <div className="fin-kpi-k">{k.k}</div>
        ...
        <div className="fin-hero">
          <div>
            <div className="fin-hero-tag">Active subscription</div>
            <div className="fin-hero-title">{FIN.subscription.plan}</div>
            <div className="fin-hero-meta">
              <div>
                Start <strong>{FIN.subscription.start}</strong>
              </div>
              <div>
                Expiry <strong>{FIN.subscription.expiry}</strong>
              </div>
              <div>
                Billing <strong>{FIN.subscription.billing}</strong>
              </div>
            </div>
          </div>
          <div className="fin-hero-actions">
            <PrimaryBtn icon="refresh">Renew subscription</PrimaryBtn>
            <GhostBtn icon="arrow-up">Upgrade plan</GhostBtn>
          </div>
        </div>
        ...
        <div className="fin-plans">
          <div className="fin-plans-title">
            ...
            <span style={{ color: "var(--fgMute)" }}>Switch any time — prorated</span>
          </div>
          <div className="fin-plans-grid">
            {FIN.plans.map((p) => (
              <div key={p.k} className={"fin-plan " + (p.current ? "current" : "")}>
                {p.current && <div className="fin-plan-yours">Your plan ✓</div>}
        ...
        <div className="fin-bot">
          <div className="r-card">
            <div className="r-card-head">
              <div className="r-card-title">Player payment history</div>
              <span className="r-card-extra">View all →</span>
            </div>
            <div>
              <div
                style={{
          ...
          <div className="r-card">
            <div className="r-card-head">
              <div className="r-card-title">Invoices</div>
              <span className="r-card-extra">Export all →</span>
            </div>
            <div>
              {FIN.invoices.map((p) => (
                <div
