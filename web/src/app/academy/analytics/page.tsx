...
const TABS = ["Growth", "Player distribution", "Coach contribution", "Weak spots"];
  ...
  return (
    <>
      <Topbar
        ...
        breadcrumb={[{ label: "Academy" }, { label: "Analytics" }]}
        actions={
          <>
            <GhostBtn icon="calendar">Last 12 months</GhostBtn>
            <GhostBtn icon="file-spreadsheet">Export CSV</GhostBtn>
            <PrimaryBtn icon="file-report">Annual report</PrimaryBtn>
          </>
        }
      />
      <Styles />
      <div className="r-page">
        <div className="an-tabs">
          {TABS.map((t) => (
            <button key={t} className={"an-tab " + (tab === t ? "on" : "")} onClick={() => setTab(t)}>
              {t}
            </button>
          ))}
        </div>
