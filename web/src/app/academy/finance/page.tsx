...
const TABS = ["Overview", "Revenue per coach", "Ledger", "Payroll", "Payments"];
  ...
  return (
    <>
      <Topbar
        ...
        breadcrumb={[{ label: "Academy" }, { label: "Finance" }]}
        actions={
          <>
            <GhostBtn icon="calendar">May 2026</GhostBtn>
            <GhostBtn icon="file-export">Export</GhostBtn>
            <PrimaryBtn icon="file-report">P&amp;L report</PrimaryBtn>
          </>
        }
      />
      <Styles />
      <div className="r-page">
        <div className="af-tabs">
          {TABS.map((t) => (
            <button key={t} className={"af-tab " + (tab === t ? "on" : "")} onClick={() => setTab(t)}>
              {t}
            </button>
          ))}
        </div>
        ...
        {tab !== "Overview" && (
          ...
          >
            ...
            <div style={{ marginTop: 10, fontWeight: 700, color: "var(--fg)", fontSize: 16 }}>{tab}</div>
            <div style={{ marginTop: 4, fontSize: 13 }}>
              Detail view for {tab.toLowerCase()} — data ready, view ships next.
            </div>
          </div>
        )}
      </div>
