  ...
  return (
    <>
      <Topbar
        ...
        breadcrumb={[{ label: t("crumb.academy") }, { label: t("nav.dashboard") }]}
        actions={
          <>
            <GhostBtn icon="calendar">{t("academy.quarter")}</GhostBtn>
            <GhostBtn icon="file-export">{t("common.exportReport")}</GhostBtn>
            <PrimaryBtn icon="sparkles">{t("academy.runEval")}</PrimaryBtn>
          </>
        }
      />
      <Styles />
        ...
        <div className="ad-mid">
          <div className="ad-board">
            <div className="ad-board-h">
              ...
              <Link href={`/${locale}/academy/coaches`} className="r-card-extra">
                View all →
              </Link>
            </div>
            {COACHES.filter((c) => !c.needsAttention)
              .slice(0, 3)
              .map((c) => (
                <Link key={c.id} href={`/${locale}/academy/coaches`} className="ad-coach-row">
